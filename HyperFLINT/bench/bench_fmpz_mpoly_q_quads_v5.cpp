// bench_fmpz_mpoly_q_quads_v5.cpp
//
// Branch I Pre-flight 8 (R14 rec #8) — 2026-05-01.
//
// Validates the *actually designed* implementation (underscore
// `_fmpz_mpoly_q_add`) against current `Rat::add` on canonical operand
// quads, NOT the high-level `fmpz_mpoly_q_add` measured by v4.  The
// reviewer flagged that the v4 +75.5% headline used a FLINT primitive
// (the high-level `fmpz_mpoly_q_add`) whose internal eager-
// canonicalisation always paths through wide-ctx — which would silently
// destroy the d=3 size-gate lever (commit e4c254879) if dropped into
// production unmodified.  The reviewer-recommended design uses the
// underscore primitive (`_fmpz_mpoly_q_add`, taking 6 separate
// fmpz_mpoly_t arguments) so HF can wrap with a narrow-ctx hoist.
//
// Methods (both on the SAME canonical operand quad, time best-of-reps):
//
//   Method A:  fmpq_mpoly_mul cross-mult chain (3 muls + 1 add) plus
//              fmpq_mpoly_gcd_cofactors.  Wide-ctx, no hoist.  This is
//              identical to v4's Method A and identical (in effect) to
//              what production Rat::add does inside the wide-ctx fast
//              path of reduce_inplace.
//
//   Method B:  _fmpz_mpoly_q_add (underscore version, FLINT 3.4.0
//              header fmpz_mpoly_q.h:138-142) on the transduced
//              fmpz_mpoly num/den.  Wide-ctx, no hoist.  The hoist
//              that production would wrap around this can only ADD
//              speedup; the comparison delta is conservative without it.
//
// FALLBACK NOTE: the prompt allows a no-hoist comparison as an
// acceptable fallback when replicating the worth_narrowing block
// proves involved (it does — it would require porting transplant
// logic from Poly::transplant).  This bench is the fallback variant.
// The hoist saves time on TOP of either method, so the underlying
// primitive's relative cost (the question Pre-flight 8 actually
// answers) is captured cleanly.
//
// CLI mirrors v4: <quads.txt> [--reps=3] [--warmup=1] [--max-quads=N].
...
// Build fmpz_mpoly_q from a fmpq_mpoly pair (same helper as v4).
//   q = (Nc * Nz) / (Dc * Dz)
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a.rfind("--reps=", 0) == 0) reps = std::atoi(a.c_str() + 7);
            ...
            std::printf("usage: bench_fmpz_mpoly_q_quads_v5 <quads.txt> "
    ...
    std::fprintf(stderr,
        "[bench_quads_v5] loaded %zu quads from %s (max=%ld)\n",
        quads.size(), path.c_str(), max_quads);
    std::fprintf(stderr,
        "[bench_quads_v5] reps=%d warmup=%d (Method B = "
        "_fmpz_mpoly_q_add underscore primitive, no hoist)\n",
        reps, warmup);
    ...
    for (size_t qi = 0; qi < quads.size(); ++qi) {
        ...
        // ---- Method B: _fmpz_mpoly_q_add (underscore primitive) ----
        //
        // Build canonical fmpz_mpoly_q operands ONCE, outside the timed
        // window (matches v4's apples-to-apples discipline).  Extract
        // num/den via numref/denref; underscore primitive operates on
        // the raw fmpz_mpoly_t pair.
        fmpz_mpoly_q_t qa, qb;
        ...
        // Result num/den.  Re-init each rep to prevent any aliasing
        // shortcut and to mirror per-call cost in production (each
        // Rat::add returns a fresh Rat).
        fmpz_mpoly_t res_num, res_den;
        fmpz_mpoly_init(res_num, ctx_z);
        fmpz_mpoly_init(res_den, ctx_z);

        const double add_B_us = time_us([&]() {
            fmpz_mpoly_zero(res_num, ctx_z);
            fmpz_mpoly_one (res_den, ctx_z);
            _fmpz_mpoly_q_add(res_num, res_den,
                              fmpz_mpoly_q_numref(qa),
                              fmpz_mpoly_q_denref(qa),
                              fmpz_mpoly_q_numref(qb),
                              fmpz_mpoly_q_denref(qb),
                              ctx_z);
        }, reps, warmup);

        // Verify equivalence: build qB_result as fmpz_mpoly_q from
        // (res_num, res_den) and q_from_A from (rn, rd); compare via
        // fmpz_mpoly_q_equal.  The underscore primitive does NOT
        // necessarily leave (res_num, res_den) in the same canonical
        // form as the high-level wrapper; canonicalise both before
        // comparison.
        fmpz_mpoly_q_t qB_result, q_from_A;
        fmpz_mpoly_q_init(qB_result, ctx_z);
        fmpz_mpoly_q_init(q_from_A,  ctx_z);
        fmpz_mpoly_set(fmpz_mpoly_q_numref(qB_result), res_num, ctx_z);
        fmpz_mpoly_set(fmpz_mpoly_q_denref(qB_result), res_den, ctx_z);
        fmpz_mpoly_q_canonicalise(qB_result, ctx_z);
        fmpz_mpoly_q_set_from_fmpq_pair(q_from_A, rn, rd, ctx_z);
        fmpz_mpoly_q_canonicalise(q_from_A, ctx_z);
        const int add_match = fmpz_mpoly_q_equal(q_from_A, qB_result, ctx_z);
        ...
        fmpz_mpoly_q_clear(qB_result, ctx_z);
        fmpz_mpoly_clear(res_num, ctx_z);
        fmpz_mpoly_clear(res_den, ctx_z);
